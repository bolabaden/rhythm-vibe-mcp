...
from lilycode_mcp.models import FallbackNoteEvent, RobustMusicFallback
...
from lilycode_mcp.utils import (
    ...
    looks_like_chordpro,
    parse_abc_headers,
    parse_abc_note_events,
    parse_chordpro_events,
    parse_chordpro_title,
)
from lilycode_mcp.chord_quality_map import looks_like_chord_token
from lilycode_mcp.duration_readable_map import duration_to_readable
...
from lilycode_mcp.app_defaults import (
    ...
    NOTATION_HINT_ABC,
    NOTATION_HINT_CHORDPRO,
    NOTATION_HINT_FREEFORM,
    NOTATION_HINT_UNKNOWN,
)
from lilycode_mcp.fallback_messages import (
...
def fallback_from_text(text: str, title: str | None = None) -> RobustMusicFallback:
    notation_hint: str = NOTATION_HINT_UNKNOWN
    ...
    if looks_like_abc(text):
        notation_hint = NOTATION_HINT_ABC
        ...
        notation_hint = NOTATION_HINT_CHORDPRO
        t = parse_chordpro_title(text)
        if t:
            title_val = t
        for chord_label, dur in parse_chordpro_events(text):
            events.append(
                FallbackNoteEvent(pitch=chord_label, duration=dur)
            )
        if events:
        ...
        notation_hint = NOTATION_HINT_FREEFORM
        for token in text.replace("\n", " ").split():
            ...
            if upper in PITCH_LETTERS:
                events.append(
                    FallbackNoteEvent(pitch=upper, duration=FALLBACK_DURATION_UNKNOWN)
                )
            elif len(t) <= 12 and looks_like_chord_token(t):
                events.append(FallbackNoteEvent(pitch=t, duration=FALLBACK_DURATION_CHORD))
        if not events:
    ...
    return RobustMusicFallback(
        ...
        meter=meter,
        tempo_bpm=tempo_bpm,
        notation_hint=cast(_NotationHintT, notation_hint),
        shorthand_text=text,
        events=events,
        warnings=warnings,
    )
...
def fallback_from_error(
    *,
    ...
    return RobustMusicFallback(
        ...
        notation_hint=NOTATION_HINT_UNKNOWN,
        warnings=[warning],
    )
